...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=556392896296:+55 63 9289-6296\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 50
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá🤗 @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* leia as regras e sinta-se em casa❤️`
				...
				teks = `finalmente a carga do grupo é reduzida 𝟭,bye bye🥳 @${num.split('@')[0]} seus serviços serão enterrados profundamente`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("Saudações KEEP DONG!! 😠")
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('você é um administrador do grupo, o bot não vai te chutar❤️')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`Link de grupo detectado, desculpe ${sender.split("@")[0]} Você será expulso do grupo em 5 segundos`)
						setTimeout( () => {
			...
			switch(command) { 
				...
				reply(`*「 PREMIUM EXPIRADO 」*\n\n➸ *ID*: ${sender.split('@')[0]}\n➸ *Premium esquerdo*: ${cekExp.days} dia(s) ${cekExp.hours} hora(s) ${cekExp.minutes} minutos(s)`)
				break
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
				...
				if (anu.error) return reply(anu.error)
					teks = `*A partir de* : ${anu.result.from}\n*Título* : ${anu.result.title}\n*Envio* : ${anu.result.uploaded}`
				...
				case 'quotes':
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length < 1) return reply(`Insira o número \ nExemplo : ${prefix}infonomor 0812345678`)
               ...
               case 'help': 
                ...
                try {
                    ...
                    console.error(err)
                    await reply(`mínimo ${len} usuário seja capaz de acessar o banco de dados`)
                }
				break
				case 'limit':
				   if (!isRegistered) return reply(ind.noregis())
                ...
                const jmla = args[1]
                if (jmla <= 1) return reply(`limite mínimo de presente é 1`)
...
• Limit: ${limitawal-updated.limit}`
            ...
            break
				...
				if ( checkATMuser(sender) >= total ) {
					...
					await reply(`*「 PAGAMENTO DE SUCESSO 」*\n\n*remetente* : Admin\n*receptor* : ${pushname}\n*nominal compra* : ${payout} \n*preço limit* : ${koinPerlimit}/limit\n*o resto dinheiro seu* : ${checkATMuser(sender)}\n\nprocess foi bem sucedido com o número do pagamento\n${createSerial(15)}`)
				} 
				break
				//no rest api 
				case 'slap':
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                const trut =['Sempre gostar mesmo qualquer um? Quanto tempo?','Se você pode ou se você quiser, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
                ...
                resul = `┏━━❉ *LEVEL* ❉━━\n┣⊱ *Nome* : ${pushname}\n┣⊱ Número : wa.me/${sender.split("@")[0]}\n┣⊱ Do utilizador XP :  ${userXp}/${requiredXp}\n┣⊱ Nível de usuário : ${userLevel}\n┗━━━━━━━━━━━━`
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'group':
					...
					if (args[0] === 'buka') {
						...
						reply(`*SUCESSO NO FECHAMENTO DO GRUPO`)
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `levo ban kkkkkk𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
				...
				case 'clone':
					...
					try {
						...
						mentions(`Foto perfil atualizado com sucesso usando a foto do perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply(ind.stikga())
					}
					break
				...
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ ATIVADO ANTILINK*')
					} else if (Number(args[0]) === 0) {
						antilink.splice(from, 1)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('*❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ DESATIVADO ANTILINK*')
					} else {
						reply(ind.satukos())
					}
					break
